...
public class BCannon extends Chess{
    public BCannon(int n, int m)
    {
        x = n;
        y = m;
        SetEval();
    ...
    public void MovAtk()
    {
        ...
        for (int i = x+1; i < 10; i++)										//向下检索
        {
            ...
            {
                for (int k = i + 1; k < 10; k++)							//继续检索到第一个非空位目标，若是敌方则可以进攻
                {
                    if (Map.RMap[k][y] <= 16 && Map.RMap[k][y] != 0)				//敌方
                    {
                        //AMap[k][y] = Map.RMap[x][y];
                        Available_Pos.add(new Node(k,y));
                        break;
                    }
                    else if (Map.RMap[k][y] >= 17)							//友方
                        break;
                }
                break;
            }
        ...
        for (int i = y+1; i < 9; i++)											//向右检索
        {
            ...
            else														//检索到第一个非空位目标
            {
                ...
                {
                    if (Map.RMap[x][k] <= 16 && Map.RMap[x][k] != 0)				//敌方
                    {
                        //AMap[x][k] = Map.RMap[x][y];
                        Available_Pos.add(new Node(x,k));
                        break;
                    }
                    else if (Map.RMap[x][k] >= 17)							//友方
                        break;
                }
                break;
            }
